  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="消息状态" prop="readStatus">
        <el-select v-model="queryParams.readStatus" placeholder="请选择消息状态" clearable>
    ...
    <el-table v-loading="loading" :data="messageList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="消息类型" align="center" prop="messageType">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.messageType === 'text'" type="primary">文本</el-tag>
          <el-tag v-else-if="scope.row.messageType === 'image'" type="success">图片</el-tag>
          <el-tag v-else-if="scope.row.messageType === 'file'" type="warning">文件</el-tag>
      ...
      <el-table-column label="消息状态" align="center" prop="readStatus">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.readStatus === 'sent'" type="info">已发送</el-tag>
          <el-tag v-else-if="scope.row.readStatus === 'delivered'" type="warning">已送达</el-tag>
          <el-tag v-else-if="scope.row.readStatus === 'read'" type="success">已读</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="发送者名称" prop="senderName">
          <el-input v-model="form.senderName" placeholder="请输入发送者名称" />
        </el-form-item>
        <el-form-item label="接收者ID" prop="receiverId">
          <el-input v-model="form.receiverId" placeholder="请输入接收者ID" />
        </el-form-item>
        <el-form-item label="接收者名称" prop="receiverName">
          <el-input v-model="form.receiverName" placeholder="请输入接收者名称" />
        </el-form-item>
        <el-form-item label="消息内容" prop="message">
          <el-input v-model="form.message" type="textarea" placeholder="请输入消息内容" />
        </el-form-item>
        <el-form-item label="消息类型" prop="messageType">
        ...
        <el-form-item label="消息状态" prop="readStatus">
          <el-select v-model="form.readStatus" placeholder="请选择消息状态">
            <el-option label="已发送" value="sent" />
            <el-option label="已送达" value="delivered" />
            <el-option label="已读" value="read" />
          </el-select>
...
<script>
import { listMessages, getMessage, delMessage, addMessage, updateMessage } from "@/api/chat";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        receiverName: [
          { required: true, message: "接收者名称不能为空", trigger: "blur" }
        ],
        message: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除聊天消息编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/chat/messages/export', {
        ...this.queryParams
